...
class DGLayer(DFALayer):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return super().forward(input)  # Note that DFA detaches activations

    def local_loss(self, error: Tensor) -> Tensor:
        dfa_loss = super().local_loss(error)
        sparsity_loss = self.sparsity_loss(self.activations)
        return dfa_loss + self.sparsity_lambda * sparsity_loss
...
class Autoencoder(pl.LightningModule):
    def __init__(self, separator_dim: int, sparsity_lambda: float, latent_dim: int, hidden_dim: int, **kwargs):
        ...
        self.encoder_l1 = DFALayer(25**2, hidden_dim, n_targets=25**2)
        self.encoder_l2 = DFALayer(hidden_dim, latent_dim, n_targets=25**2)
        ...
        self.decoder_l1 = DFALayer(latent_dim, hidden_dim, n_targets=25**2)
